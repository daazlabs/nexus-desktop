...
const DESKTOP_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'delegar_tarefa',
      description: 'Delegate a token-heavy sub-task (extensive research, summarizing a lot of content, bulk data processing) to a cheaper/faster model, and get back only the condensed result — use this instead of doing the heavy work yourself when you are a paid/premium model, so the user is not paying premium rates for bulk legwork. Do NOT use for simple questions you can just answer directly. The task description must be fully self-contained: the model that executes it does not see the rest of this conversation, only what you write here.',
      parameters: {
        type: 'object',
        properties: {
          tarefa: { type: 'string', description: 'Complete, self-contained description of the sub-task to perform — include all context/URLs/text needed, since the executing model has no access to this conversation.' },
          max_tokens: { type: 'number', description: 'Optional cap on the response length (default 1500, max 4000).' },
        },
        required: ['tarefa'],
      },
    },
