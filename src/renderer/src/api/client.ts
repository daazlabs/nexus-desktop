...
export const api = {
  ...
  listConnectors: async (): Promise<{ id: string; name: string; authMethodSupported: string; status: string; available: boolean; lastError?: string; account?: string }[]> => {
  ...
  setN8nCredentials: async (baseUrl: string, apiKey: string) => {
    return await nexusApi.connectors?.setN8nCredentials?.(baseUrl, apiKey)
  },
